...
class Aluno(models.Model):
    ...
    STATUS_TYPE = [
        ('AT','ATIVO'),
        ('AR','AGUARDANDO ROTA OFICIAL'),
        ('AR','AGUARDANDO ROTA TESTE'),
        ('IN','INATIVO'),
    ]

    nome = models.CharField('Nome do aluno', max_length=50)
    address = models.ForeignKey('endereco.Endereco', on_delete=models.SET_NULL, null=True, blank=True)
    ...
    escola = models.ForeignKey('escola.Escola', on_delete=models.SET_NULL, null=True, blank=True, default=None)
    status = models.CharField('Status', max_length=50, choices=STATUS_TYPE, default="IN")
